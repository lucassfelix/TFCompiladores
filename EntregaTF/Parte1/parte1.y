    ...
    { TS_entry nodo = currEscopo.pesquisa($1, currClass);
      ...
      else 
      {
        if(currentType.getTipo() == Tp_ARRAY)
          currEscopo.insert(new TS_entry($1, currentType, currClass,0,currentType.getTipoBase())); 
        else
...
exp 
  ...
  | IDENT       
    { TS_entry nodo = currEscopo.pesquisa($1, currClass);
      if (nodo == null) {
          nodo = ts.pesquisa($1, currClass);
          if(nodo == null)
          {
            yyerror("(sem) var <" + $1 + "> da classe <" +currClass.name()+"> nao declarada neste escopo"); 
            $$ = Tp_ERRO;  
          }
          else
          {
  ...
  | exp '.' {currClass = ClasseID.CampoStruct;} exp {currClass = ClasseID.ANY;    
      if ( ((TS_entry)$4).getClasse() != ClasseID.CampoStruct)
      {
        yyerror("(sem) <" + ((TS_entry)$4).getId()+"> nao eh variavel campo de struct.");
        $$ = Tp_ERRO;
      }
      ...
      {
        TS_entry aux = ((TS_entry)$1).getTabSimb().pesquisa(((TS_entry)$4).getId(),ClasseID.CampoStruct);
        if(aux == null)
        {
          yyerror("(sem) <" + ((TS_entry)$4).getId() + "> nao eh campo da struct <" + ((TS_entry)$1).getId()  +">");
          $$ = Tp_ERRO;
        }
        else
          $$ = ((TS_entry)$4).getTipo();
  ...
  ')'
    {
      ...
      {
        yyerror("(sem) <" + ((TS_entry)$1).getId() +"> nao eh um nome valido de funcao cont : " + contadorArgumento.peek().size());
        $$ = Tp_ERRO;
      } 
      else
      {
        ...
        {
          yyerror("(sem) <" + ((TS_entry)$1).getId() + "> requer " + params.size()+ " argumento(s), porem recebeu " + contadorArgumento.peek().size());
          $$ = Tp_ERRO;
        }
        else{
...
lista_exp
  : lista_exp ',' exp {contadorArgumento.peek().add(((TS_entry)$3));}
  | exp {contadorArgumento.peek().add(((TS_entry)$1));}
  | //Vazio 
  ;
