...
%token WHILE,TRUE, FALSE, IF, ELSE, BREAK, CONTINUE, INCR, FOR, DECR
...
decl: type tarray ID ';'{ TS_entry nodo = ts.pesquisa($3);
						if (nodo != null)
							yyerror("(sem) variavel >" + $3 + "< jah declarada");
						else ts.insert(new TS_entry($3, $1, $2, $1));}
    ;
...
cmd :   exp ';' {System.out.println("\tPOPL %EDX");}
    ...
    | BREAK  ';' 	{System.out.printf("\tJMP rot_%02d",(int)pRotRep.peek() + 1);}
	| CONTINUE ';' {System.out.printf("\tJMP rot_%02d",pRotRep.peek());}
    | WHILE {
	...
	| FOR {pRotRep.push(proxRot); proxRot += 4;} 
	'(' expOpc ';'
			{
				System.out.printf("rot_%02d:\n",pRotRep.peek()+3);
			}
...
expOpc : exp | ;
expOpc2 : exp | {System.out.println("\tPUSHL $1");};
...
exp :  NUM  { System.out.println("\tPUSHL $"+$1); } 
	...
	| DECR ID { System.out.println("\tPUSHL _"+$2);
				System.out.println("\tPOPL %EDX");
				System.out.println("\tADDL $-1, %EDX" );
				System.out.println("\tPUSHL %EDX");
  				System.out.println("\tMOVL %EDX, _"+$2);}

	| ID DECR { System.out.println("\tPUSHL _"+$1);
				System.out.println("\tPUSHL _"+$1);
				System.out.println("\tPOPL %EDX");
				System.out.println("\tADDL $-1, %EDX" );
  				System.out.println("\tMOVL %EDX, _"+$1);}
	...
	| ID ADDATRIB exp {System.out.println("\tPUSHL _"+$1);
					   gcExpArit('+');
					   System.out.println("\tPOPL %EDX");
					   System.out.println("\tMOVL %EDX, _"+$1);
					System.out.println("\tPUSHL %EDX");
	...
	| ID '[' exp ']'
	{
		...
		gcExpArit('*');
		System.out.println("\tPOPL %EDX");
		System.out.println("\tMOVL _" +$1 + "( %EDX ) , %EAX");
